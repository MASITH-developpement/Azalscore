...
// ============================================================================
// VUE DÉTAIL ARTICLE - BaseViewStandard
// ============================================================================
...
const ProductDetailView: React.FC<ProductDetailViewProps> = ({ productId, onBack }) => {
  ...
  const tabs: TabDefinition<Product>[] = [
    ...
    },
    {
      ...
      label: 'Stock',
      icon: <MapPin size={16} />,
      badge: product.stock_by_location?.length || 0,
    ...
    },
    {
      ...
      component: ProductFinancialTab
    },
    {
      id: 'docs',
      label: 'Documents',
      icon: <ClipboardList size={16} />,
      badge: product.documents?.length || 0,
      component: ProductDocsTab
    },
    {
      id: 'history',
      label: 'Historique',
    ...
    },
    {
      ...
      component: ProductIATab
    }
  ];

  // Barre d'info
  const stockLevel = getStockLevel(product);
  const stockColorMap: Record<string, SemanticColor> = {
    danger: 'red',
    warning: 'orange',
    success: 'green',
    info: 'blue'
  ...
  const infoBarItems: InfoBarItem[] = [
    ...
    },
    {
      ...
      valueColor: stockColorMap[stockLevel]
    },
    {
      id: 'status',
      label: 'Niveau',
      value: getStockLevelLabel(product),
      valueColor: stockColorMap[stockLevel]
    }
  ];
      ...
      items: [
        {
          ...
          label: 'Stock actuel',
          value: formatQuantity(product.current_stock, product.unit),
          highlight: isLowStock(product) || isOutOfStock(product)
        },
        {
          id: 'reserved',
          label: 'Réservé',
          value: formatQuantity(product.reserved_stock || 0, product.unit)
        },
        {
          id: 'available',
          label: 'Disponible',
          value: formatQuantity(product.available_stock ?? product.current_stock, product.unit),
          highlight: true
        }
      ]
    },
    {
      ...
      items: [
        ...
        },
        {
          id: 'value',
          label: 'Valeur stock',
          value: formatCurrency(product.current_stock * product.cost_price),
          format: 'currency',
          highlight: true
        }
      ]
  ...
  return (
    <BaseViewStandard<Product>
      title={product.name}
      subtitle={`Article ${product.code}`}
      status={{
        label: getStockLevelLabel(product),
        color: statusColorMap[stockLevel]
      }}
      data={product}
      view="detail"
      tabs={tabs}
...
const ProductsView: React.FC<ProductsViewProps> = ({ onSelectProduct }) => {
  ...
  const columns: TableColumn<Product>[] = [
    ...
    { id: 'unit', header: 'Unité', accessor: 'unit' },
    {
      ...
      render: (v, row: Product) => (
        <span className={isOutOfStock(row) ? 'text-red-600 font-semibold' : isLowStock(row) ? 'text-orange-600 font-semibold' : ''}>
    ...
    },
    {
      ...
      render: (_, row: Product) => (
        <Button size="sm" variant="ghost" leftIcon={<Eye size={14} />} onClick={() => onSelectProduct(row.id)}>
          Voir
        </Button>
      )
    }
...
const MovementsView: React.FC = () => {
  ...
  const columns: TableColumn<Movement>[] = [
    ...
    { id: 'date', header: 'Date', accessor: 'date', render: (v) => formatDate(v as string) },
    {
      ...
      accessor: 'type',
      render: (v) => {
        const config = MOVEMENT_TYPE_CONFIG[v as MovementType];
    ...
    { id: 'dest_location_name', header: 'Destination', accessor: 'dest_location_name', render: (v) => (v as string) || '-' },
    {
      ...
      accessor: 'status',
      render: (v) => {
        const config = MOVEMENT_STATUS_CONFIG[v as MovementStatus];
        return <Badge color={config.color}>{config.label}</Badge>;
      }
    },
    {
        ...
        <form onSubmit={handleSubmit}>
          <Grid cols={2}>
            <div className="azals-field">
              ...
              <Select
                value={formData.type || ''}
                onChange={(v) => setFormData({ ...formData, type: v as MovementType })}
                options={Object.entries(MOVEMENT_TYPE_CONFIG).map(([value, config]) => ({
                  value,
                  label: config.label
                }))}
              />
...
const PickingsView: React.FC = () => {
  ...
  const columns: TableColumn<Picking>[] = [
    ...
    { id: 'date', header: 'Date', accessor: 'date', render: (v) => formatDate(v as string) },
    {
      ...
      accessor: 'type',
      render: (v) => {
        const config = PICKING_TYPE_CONFIG[v as keyof typeof PICKING_TYPE_CONFIG];
        return <Badge color={config.color}>{config.label}</Badge>;
      }
    },
    { id: 'reference', header: 'Référence', accessor: 'reference', render: (v) => (v as string) || '-' },
    {
      ...
      accessor: 'status',
      render: (v) => {
        const config = PICKING_STATUS_CONFIG[v as keyof typeof PICKING_STATUS_CONFIG];
        return <Badge color={config.color}>{config.label}</Badge>;
      }
    }
  ];
  ...
  const columns: TableColumn<InventoryCount>[] = [
    ...
    { id: 'warehouse_name', header: 'Entrepôt', accessor: 'warehouse_name' },
    {
      ...
      render: (v) => {
        ...
        const config = statuses[v as string] || { label: v, color: 'gray' };
        return <Badge color={config.color}>{config.label}</Badge>;
      }
    }
  ];
...
const StockModule: React.FC = () => {
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'products':
        return <ProductsView onSelectProduct={handleSelectProduct} />;
      case 'warehouses':
