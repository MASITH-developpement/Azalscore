/**
 * AZALSCORE Module - FACTURES - Risk Tab
 * Onglet Analyse de Risque pour le client de la facture
 *
 * Affiche:
 * - Score de risque externe (SIREN/SIRET via Pappers/INSEE)
 * - Score interne (historique de paiement du client)
...
/**
 * FactureRiskTab - Onglet Analyse de Risque
 */
export const FactureRiskTab: React.FC<TabContentProps<Facture>> = ({ data: facture }) => {
  const [showDetails, setShowDetails] = useState(true);

  // Recuperer SIREN ou SIRET du client
  const identifier = facture.customer_siret || facture.customer_siren || '';
  ...
  return (
    ...
    >
      <div className="azals-std-tab-content">
        ...
        <Grid cols={2} gap="lg">
          {/* Score interne (historique paiement) */}
          <Card title="Score Client Interne" icon={<CreditCard size={18} />}>
            <InternalScoreDisplay customerId={facture.customer_id} />
          </Card>

          {/* Score externe (SIREN/SIRET) */}
