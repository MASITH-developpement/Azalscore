...
export const CustomerDocsTab: React.FC<TabContentProps<Customer>> = ({ data: customer }) => {
  ...
  const handleExportPdf = (): void => {
    window.dispatchEvent(new CustomEvent('azals:document:download', {
      detail: { type: 'customer', id: customer.id, format: 'pdf' }
    }));
  };

  const handleUpload = (): void => {
    window.dispatchEvent(new CustomEvent('azals:document:upload', {
      detail: { type: 'customer', id: customer.id }
    }));
  };
  ...
  const handleOpenContractDoc = (docType: string): void => {
    window.dispatchEvent(new CustomEvent('azals:customer:open-contract-doc', {
      detail: { customerId: customer.id, docType }
    }));
  };
  ...
  return (
    <div className="azals-std-tab-content">
      ...
      >
        <div className="azals-linked-docs">
          <button className="azals-linked-doc azals-linked-doc--contract" onClick={() => { handleOpenContractDoc('contrats'); }}>
            <FileText size={16} />
            <span>Contrats</span>
          </button>
          <button className="azals-linked-doc azals-linked-doc--contract" onClick={() => { handleOpenContractDoc('cgv'); }}>
            <FileText size={16} />
            <span>CGV signées</span>
          </button>
          <button className="azals-linked-doc azals-linked-doc--contract" onClick={() => { handleOpenContractDoc('mandats-sepa'); }}>
            <FileText size={16} />
