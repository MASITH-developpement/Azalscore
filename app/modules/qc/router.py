"""
AZALS MODULE T4 - Router API Contrôle Qualité
===============================================
...
def get_service(
    ...
    """Obtient le service QC pour le tenant courant."""
    return get_qc_service(db, current_user.tenant_id)
...
def get_rule(rule_id: int, service: QCService = Depends(get_service)):
...
def delete_rule(rule_id: int, service: QCService = Depends(get_service)):
...
def get_module(module_id: int, service: QCService = Depends(get_service)):
...
def get_module_by_code(module_code: str, service: QCService = Depends(get_service)):
...
def get_module_scores(module_id: int, service: QCService = Depends(get_service)):
...
def get_validation(validation_id: int, service: QCService = Depends(get_service)):
...
def get_latest_metrics(service: QCService = Depends(get_service)):
    """Récupère les dernières métriques QC."""
    metrics = service.get_metrics_history(limit=1)
...
def list_dashboards(
    ...
    return service.list_dashboards(owner_id=current_user.id)


@router.get("/dashboards/{dashboard_id}", response_model=QCDashboardResponse)
def get_dashboard(dashboard_id: int, service: QCService = Depends(get_service)):
...
def get_dashboard_data(dashboard_id: int, service: QCService = Depends(get_service)):
...
def get_default_dashboard_data(service: QCService = Depends(get_service)):
...
def get_template(template_id: int, service: QCService = Depends(get_service)):
...
def apply_template(
    ...
    rules = service.apply_template(template_id, created_by=current_user.id)
    if not rules:
...
def get_qc_stats(service: QCService = Depends(get_service)):
