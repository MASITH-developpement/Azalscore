"""
AZALS MODULE M2A - API Router Comptabilité Automatisée Unifié
==============================================================

Endpoints REST unifiés pour le module de comptabilité automatisée.
...
def get_document_service(
    ...
    return DocumentService(db, context.tenant_id, str(context.user_id))

def get_dashboard_service(
    db: Session = Depends(get_db),
    context: SaaSContext = Depends(get_context)
) -> DashboardService:
    return DashboardService(db, context.tenant_id, str(context.user_id))

def get_bank_service(
    db: Session = Depends(get_db),
    context: SaaSContext = Depends(get_context)
) -> BankPullService:
    return BankPullService(db, context.tenant_id, str(context.user_id))

def get_reconciliation_service(
    db: Session = Depends(get_db),
    context: SaaSContext = Depends(get_context)
) -> ReconciliationService:
    return ReconciliationService(db, context.tenant_id, str(context.user_id))

def get_auto_accounting_service(
    db: Session = Depends(get_db),
    context: SaaSContext = Depends(get_context)
) -> AutoAccountingService:
    return AutoAccountingService(db, context.tenant_id, str(context.user_id))
...
async def list_alerts(
    ...
    query = db.query(AccountingAlert).filter(
        AccountingAlert.tenant_id == context.tenant_id
    )
...
async def get_ai_performance(
    ...
    service = AIClassificationService(db, context.tenant_id, str(context.user_id))
