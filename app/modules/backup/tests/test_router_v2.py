...
def test_create_config_success(test_client, client, backup_config_data, backup_config, mock_backup_service):
    ...
    response = test_client.post("/v2/backup/config", json=backup_config_data)
...
def test_create_config_already_exists(test_client, client, backup_config_data, backup_config, mock_backup_service):
...
def test_get_config_success(test_client, client, backup_config, mock_backup_service):
    """Test récupération configuration backup avec succès"""
    mock_backup_service.get_config.return_value = MagicMock(**backup_config)

    response = test_client.get("/v2/backup/config")
...
def test_get_config_not_found(test_client, client, mock_backup_service):
...
def test_update_config_success(test_client, client, backup_config, mock_backup_service):
    ...
    response = test_client.patch("/v2/backup/config", json=update_data)
...
def test_update_config_not_found(test_client, client, mock_backup_service):
    """Test mise à jour configuration backup - non trouvée"""
    mock_backup_service.update_config.return_value = None

    response = test_client.patch("/v2/backup/config", json={"frequency": "WEEKLY"})
...
def test_create_backup_success(test_client, client, backup_data, backup, mock_backup_service):
    """Test création backup manuel avec succès"""
    mock_backup_service.create_backup.return_value = MagicMock(**backup)

    response = test_client.post("/v2/backup", json=backup_data)
...
def test_create_backup_no_config(test_client, client, backup_data, mock_backup_service):
...
def test_list_backups_success(test_client, client, backup_list, mock_backup_service):
    ...
    response = test_client.get("/v2/backup")
...
def test_list_backups_with_status_filter(test_client, client, backup_list, mock_backup_service):
    ...
    response = test_client.get("/v2/backup?status=COMPLETED")
...
def test_list_backups_with_pagination(test_client, client, backup_list, mock_backup_service):
    """Test liste des backups avec pagination"""
    mock_backup_service.list_backups.return_value = ([MagicMock(**backup_list[0])], 1)

    response = test_client.get("/v2/backup?skip=0&limit=1")
...
def test_get_backup_success(test_client, client, backup, mock_backup_service):
    """Test récupération d'un backup avec succès"""
    mock_backup_service.get_backup.return_value = MagicMock(**backup)

    response = test_client.get(f"/v2/backup/{backup['id']}")
...
def test_get_backup_not_found(test_client, client, mock_backup_service):
    """Test récupération d'un backup - non trouvé"""
    mock_backup_service.get_backup.return_value = None

    response = test_client.get("/v2/backup/fake-backup-id")

    assert response.status_code == 404
    assert "Sauvegarde non trouvée" in response.json()["detail"]


def test_delete_backup_success(test_client, client, backup, mock_backup_service):
    ...
    response = test_client.delete(f"/v2/backup/{backup['id']}")
...
def test_delete_backup_not_found(test_client, client, mock_backup_service):
    """Test suppression d'un backup - non trouvé"""
    mock_backup_service.get_backup.return_value = None

    response = test_client.delete("/v2/backup/fake-backup-id")

    assert response.status_code == 404
    assert "Sauvegarde non trouvée" in response.json()["detail"]


def test_run_backup_success(test_client, client, backup, mock_backup_service):
    ...
    response = test_client.post(f"/v2/backup/{backup['id']}/run")
...
def test_run_backup_reference_not_found(test_client, client, mock_backup_service):
    """Test exécution d'un backup - référence non trouvée"""
    mock_backup_service.get_backup.return_value = None

    response = test_client.post("/v2/backup/fake-backup-id/run")
...
def test_restore_backup_success(test_client, client, restore_request_data, restore_log, mock_backup_service):
    """Test restauration d'un backup avec succès"""
    mock_backup_service.restore_backup.return_value = MagicMock(**restore_log)

    response = test_client.post("/v2/backup/restore", json=restore_request_data)
...
def test_restore_backup_not_found(test_client, client, restore_request_data, mock_backup_service):
...
def test_restore_backup_invalid_status(test_client, client, restore_request_data, mock_backup_service):
...
def test_get_dashboard_success(test_client, client, backup_dashboard, mock_backup_service):
    ...
    response = test_client.get("/v2/backup/dashboard/stats")
...
def test_get_dashboard_no_config(test_client, client, backup_dashboard, mock_backup_service):
