"""
AZALS - IAM Router (CRUDRouter v3)
===================================

Router complet pour le module IAM (Identites & Acces).
...
from .schemas import (
    GroupCreate, GroupListResponse, GroupMembership, GroupResponse,
    InvitationAccept, InvitationCreate, InvitationResponse,
    LoginRequest, LoginResponse, LogoutRequest,
    MFADisable, MFASetupResponse, MFAVerify,
    PasswordChange, PasswordPolicyResponse, PasswordPolicyUpdate,
    PermissionCheck, PermissionCheckResult, PermissionListResponse, PermissionResponse,
...
router = APIRouter(prefix="/iam", tags=["IAM - Identites & Acces"])
...
def get_service_public(
    db: Session = Depends(get_db),
    tenant_id: str = Depends(get_tenant_id)
) -> IAMService:
    """Dependance pour obtenir le service IAM (endpoints PUBLICS)."""
    return get_iam_service(db, tenant_id)


def get_service_protected(
    db: Session = Depends(get_db),
    context: SaaSContext = Depends(get_context)
) -> IAMService:
    """Dependance pour obtenir le service IAM (endpoints PROTEGES)."""
...
def _build_user_response(user) -> UserResponse:
    """Construit UserResponse depuis un User model."""
    return UserResponse(
        id=user.id,
...
def _build_role_response(role) -> RoleResponse:
    """Construit RoleResponse depuis un Role model."""
    return RoleResponse(
        id=role.id,
...
async def login(
    request: Request,
    data: LoginRequest,
    service: IAMService = Depends(get_service_public)
):
    """Authentification utilisateur. Retourne access_token et refresh_token."""
...
async def refresh_token(
    ...
    """Rafraichit les tokens avec le refresh_token."""
    ip_address = request.client.host if request.client else None
...
async def list_users(
    ...
    users, total = service.list_users(
        ...
        is_active=is_active, search=search, role_code=role_code
    )
...
async def delete_user(
    ...
    """Supprime un utilisateur (desactivation)."""
...
async def get_role(
    ...
    if not role:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Role non trouve")
    return _build_role_response(role)
...
async def revoke_role(
    ...
    """Retire un role a un utilisateur."""
...
async def list_permissions(
    ...
    return PermissionListResponse(
        items=[
            PermissionResponse(
                id=p.id, tenant_id=p.tenant_id, code=p.code, module=p.module,
                resource=p.resource, action=p.action.value, name=p.name,
                description=p.description, is_system=p.is_system,
                is_active=p.is_active, is_dangerous=p.is_dangerous,
...
async def create_group(
    ...
    try:
        ...
        return GroupResponse(
            id=group.id, tenant_id=group.tenant_id, code=group.code,
            name=group.name, description=group.description,
            is_active=group.is_active, user_count=len(group.users),
            roles=[r.code for r in group.roles], created_at=group.created_at
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def list_groups(
    ...
    return GroupListResponse(
        items=[
            GroupResponse(
                id=g.id, tenant_id=g.tenant_id, code=g.code,
                name=g.name, description=g.description,
                is_active=g.is_active, user_count=len(g.users),
                roles=[r.code for r in g.roles], created_at=g.created_at
            )
            for g in groups
        ],
        total=len(groups)
...
async def add_group_members(
    ...
    """Ajoute des utilisateurs a un groupe."""
...
async def setup_mfa(
    ...
    """Configure MFA pour l'utilisateur connecte."""
...
async def disable_mfa(
    ...
    if not service.disable_mfa(context.user_id, data.password, data.code):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Mot de passe ou code MFA invalide")
...
async def create_invitation(
    ...
    try:
        ...
        return InvitationResponse(
            ...
            token=invitation.token, status=invitation.status.value,
            roles_to_assign=[], groups_to_assign=[],
            created_at=invitation.created_at, expires_at=invitation.expires_at,
            invited_by=invitation.invited_by, accepted_at=invitation.accepted_at
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def accept_invitation(
    ...
    """Accepte une invitation et cree le compte (endpoint PUBLIC)."""
...
async def list_my_sessions(
    ...
    """Liste les sessions de l'utilisateur connecte."""
    ...
    return SessionListResponse(
        items=[
            SessionResponse(
                id=s.id, token_jti=s.token_jti, ip_address=s.ip_address,
                user_agent=s.user_agent, status=s.status.value,
                created_at=s.created_at, expires_at=s.expires_at,
                last_activity_at=s.last_activity_at, is_current=False
            )
            for s in sessions
        ],
        total=len(sessions)
...
async def get_password_policy(
    ...
    """Recupere la politique de mot de passe."""
...
async def update_password_policy(
    ...
    """Met a jour la politique de mot de passe."""
    policy = service._get_password_policy()
