"""
AZALS MODULE - PROCUREMENT: Router Unifié
==========================================
...
Enregistrement dans main.py:
    from app.modules.procurement.router_unified import router as procurement_router

    # Double enregistrement pour compatibilité
    app.include_router(procurement_router, prefix="/v2")
    app.include_router(procurement_router, prefix="/v1", deprecated=True)

Conformité : AZA-NF-006

ENDPOINTS (38 total):
- Suppliers (7): CRUD + approve + contacts
- Requisitions (6): CRUD + submit/approve/reject
- Orders (11): CRUD + send/confirm/validate + delete + create-invoice + export
- Receipts (3): list + create + validate
- Invoices (7): CRUD + validate + delete + export
- Payments (1): create
...
router = APIRouter(prefix="/procurement", tags=["Procurement - Approvisionnement"])
...
def list_suppliers(
    supplier_status: SupplierStatus | None = Query(None, alias="status"),
...
def approve_supplier(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    supplier = service.approve_supplier(supplier_id, context.user_id)
    if not supplier:
        raise HTTPException(status_code=404, detail="Supplier not found")
    return supplier
...
def create_requisition(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    return service.create_requisition(data, context.user_id)

@router.get("/requisitions")
def list_requisitions(
    requisition_status: RequisitionStatus | None = Query(None, alias="status"),
...
def approve_requisition(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    requisition = service.approve_requisition(requisition_id, context.user_id)
    if not requisition:
...
def reject_requisition(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    requisition = service.reject_requisition(requisition_id, context.user_id, reason)
    if not requisition:
        raise HTTPException(status_code=400, detail="Requisition not found or not submitted")
    return requisition
...
def create_purchase_order(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    return service.create_purchase_order(data, context.user_id)
...
def update_purchase_order(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    order = service.update_purchase_order(order_id, data, context.user_id)
    if not order:
...
def validate_purchase_order(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    order = service.validate_purchase_order(order_id, context.user_id)
    if not order:
        raise HTTPException(status_code=400, detail="Order not found or not in DRAFT status")
    return order
...
def create_invoice_from_order(
    ...
    invoice = service.create_invoice_from_order(
        order_id, context.user_id, invoice_date, supplier_invoice_number
    )
    if not invoice:
...
def create_goods_receipt(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    try:
        return service.create_goods_receipt(data, context.user_id)
...
def validate_goods_receipt(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    receipt = service.validate_goods_receipt(receipt_id, context.user_id)
    if not receipt:
...
def create_purchase_invoice(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    return service.create_purchase_invoice(data, context.user_id)

@router.get("/invoices", response_model=PurchaseInvoiceList)
def list_purchase_invoices(
    supplier_id: UUID | None = None,
    invoice_status: PurchaseInvoiceStatus | None = Query(None, alias="status"),
    start_date: date | None = None,
    end_date: date | None = None,
    skip: int = Query(0, ge=0),
    limit: int = Query(50, ge=1, le=200),
...
def update_purchase_invoice(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    invoice = service.update_purchase_invoice(invoice_id, data, context.user_id)
    if not invoice:
...
def validate_purchase_invoice(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    invoice = service.validate_purchase_invoice(invoice_id, context.user_id)
    if not invoice:
...
def create_supplier_payment(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    return service.create_supplier_payment(data, context.user_id)
...
def create_supplier_evaluation(
    ...
    service = get_procurement_service(db, context.tenant_id, context.user_id)
    return service.create_supplier_evaluation(data, context.user_id)
