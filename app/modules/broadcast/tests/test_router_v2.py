...
def test_create_template_success(client, template_data, mock_broadcast_service):
...
def test_create_template_duplicate_code(client, template_data, mock_broadcast_service):
...
def test_list_templates(client, template_data, mock_broadcast_service):
    """Test liste des templates."""
    mock_broadcast_service.list_templates.return_value = ([template_data], 1)

    response = client.get("/v2/broadcast/templates")
...
def test_list_templates_with_filters(client, mock_broadcast_service):
...
def test_get_template(client, template_data, mock_broadcast_service):
    """Test récupération d'un template."""
    mock_broadcast_service.get_template.return_value = template_data

    response = client.get("/v2/broadcast/templates/1")
...
def test_get_template_not_found(client, mock_broadcast_service):
    """Test récupération d'un template inexistant."""
    mock_broadcast_service.get_template.return_value = None

    response = client.get("/v2/broadcast/templates/999")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_update_template(client, template_data, mock_broadcast_service):
...
def test_update_template_not_found(client, mock_broadcast_service):
...
def test_delete_template(client, mock_broadcast_service):
    """Test suppression d'un template."""
    mock_broadcast_service.delete_template.return_value = True

    response = client.delete("/v2/broadcast/templates/1")

    assert response.status_code == status.HTTP_204_NO_CONTENT


def test_delete_template_not_found(client, mock_broadcast_service):
    """Test suppression d'un template inexistant."""
    mock_broadcast_service.delete_template.return_value = False

    response = client.delete("/v2/broadcast/templates/999")
...
def test_create_recipient_list(client, recipient_list_data, mock_broadcast_service):
...
def test_list_recipient_lists(client, recipient_list_data, mock_broadcast_service):
    """Test liste des listes de destinataires."""
    mock_broadcast_service.list_recipient_lists.return_value = ([recipient_list_data], 1)

    response = client.get("/v2/broadcast/recipient-lists")
...
def test_list_recipient_lists_with_pagination(client, mock_broadcast_service):
...
def test_get_recipient_list(client, recipient_list_data, mock_broadcast_service):
    """Test récupération d'une liste."""
    mock_broadcast_service.get_recipient_list.return_value = recipient_list_data

    response = client.get("/v2/broadcast/recipient-lists/1")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["id"] == 1


def test_get_recipient_list_not_found(client, mock_broadcast_service):
    """Test récupération d'une liste inexistante."""
    mock_broadcast_service.get_recipient_list.return_value = None

    response = client.get("/v2/broadcast/recipient-lists/999")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_add_member_to_list(client, recipient_member_data, recipient_list_data, mock_broadcast_service):
...
def test_add_member_list_not_found(client, mock_broadcast_service):
...
def test_list_members(client, recipient_member_data, mock_broadcast_service):
    """Test liste des membres d'une liste."""
    mock_broadcast_service.get_list_members.return_value = ([recipient_member_data], 1)

    response = client.get("/v2/broadcast/recipient-lists/1/members")
...
def test_list_members_with_filters(client, mock_broadcast_service):
...
def test_delete_member(client, mock_broadcast_service):
    """Test suppression d'un membre."""
    mock_broadcast_service.remove_member_from_list.return_value = True

    response = client.delete("/v2/broadcast/recipient-lists/1/members/1")

    assert response.status_code == status.HTTP_204_NO_CONTENT


def test_delete_member_not_found(client, mock_broadcast_service):
    """Test suppression d'un membre inexistant."""
    mock_broadcast_service.remove_member_from_list.return_value = False

    response = client.delete("/v2/broadcast/recipient-lists/1/members/999")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_add_member_external_email(client, recipient_list_data, mock_broadcast_service):
...
def test_create_scheduled_broadcast(client, broadcast_data, mock_broadcast_service):
...
def test_list_scheduled_broadcasts(client, broadcast_data, mock_broadcast_service):
    """Test liste des diffusions programmées."""
    mock_broadcast_service.list_scheduled_broadcasts.return_value = ([broadcast_data], 1)

    response = client.get("/v2/broadcast/scheduled")
...
def test_list_scheduled_broadcasts_with_status_filter(client, mock_broadcast_service):
...
def test_list_scheduled_broadcasts_with_frequency_filter(client, mock_broadcast_service):
...
def test_get_scheduled_broadcast(client, broadcast_data, mock_broadcast_service):
    """Test récupération d'une diffusion."""
    mock_broadcast_service.get_scheduled_broadcast.return_value = broadcast_data

    response = client.get("/v2/broadcast/scheduled/1")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["id"] == 1


def test_get_scheduled_broadcast_not_found(client, mock_broadcast_service):
    """Test récupération d'une diffusion inexistante."""
    mock_broadcast_service.get_scheduled_broadcast.return_value = None

    response = client.get("/v2/broadcast/scheduled/999")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_update_scheduled_broadcast(client, broadcast_data, mock_broadcast_service):
...
def test_update_scheduled_broadcast_not_found(client, mock_broadcast_service):
...
def test_activate_broadcast(client, broadcast_data, mock_broadcast_service):
    ...
    response = client.post("/v2/broadcast/scheduled/1/activate")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["status"] == "ACTIVE"


def test_activate_broadcast_not_found(client, mock_broadcast_service):
    """Test activation d'une diffusion inexistante."""
    mock_broadcast_service.activate_broadcast.return_value = None

    response = client.post("/v2/broadcast/scheduled/999/activate")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_pause_broadcast(client, broadcast_data, mock_broadcast_service):
    ...
    response = client.post("/v2/broadcast/scheduled/1/pause")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["status"] == "PAUSED"


def test_pause_broadcast_not_found(client, mock_broadcast_service):
    """Test mise en pause d'une diffusion inexistante."""
    mock_broadcast_service.pause_broadcast.return_value = None

    response = client.post("/v2/broadcast/scheduled/999/pause")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_cancel_broadcast(client, broadcast_data, mock_broadcast_service):
    ...
    response = client.post("/v2/broadcast/scheduled/1/cancel")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["status"] == "CANCELLED"


def test_cancel_broadcast_not_found(client, mock_broadcast_service):
    """Test annulation d'une diffusion inexistante."""
    mock_broadcast_service.cancel_broadcast.return_value = None

    response = client.post("/v2/broadcast/scheduled/999/cancel")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_execute_broadcast_now(client, execution_data, mock_broadcast_service):
...
def test_execute_broadcast_not_found(client, mock_broadcast_service):
...
def test_list_executions(client, execution_data, mock_broadcast_service):
    """Test liste des exécutions."""
    mock_broadcast_service.list_executions.return_value = ([execution_data], 1)

    response = client.get("/v2/broadcast/executions")
...
def test_list_executions_with_filters(client, mock_broadcast_service):
...
def test_get_execution(client, execution_data, mock_broadcast_service):
    """Test récupération d'une exécution."""
    mock_broadcast_service.get_execution.return_value = execution_data

    response = client.get("/v2/broadcast/executions/1")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["id"] == 1


def test_get_execution_not_found(client, mock_broadcast_service):
    """Test récupération d'une exécution inexistante."""
    mock_broadcast_service.get_execution.return_value = None

    response = client.get("/v2/broadcast/executions/999")

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_get_execution_details(client, delivery_detail_data, mock_broadcast_service):
    """Test récupération des détails d'exécution."""
    mock_broadcast_service.get_delivery_details.return_value = ([delivery_detail_data], 1)

    response = client.get("/v2/broadcast/executions/1/details")
...
def test_get_execution_details_with_filters(client, mock_broadcast_service):
...
def test_get_preferences(client, preference_data, mock_broadcast_service):
    """Test récupération des préférences."""
    mock_broadcast_service.get_user_preferences.return_value = preference_data

    response = client.get("/v2/broadcast/preferences")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["user_id"] == 123


def test_get_preferences_create_default(client, preference_data, mock_broadcast_service):
...
def test_update_preferences(client, preference_data, mock_broadcast_service):
...
def test_update_preferences_with_channel(client, preference_data, mock_broadcast_service):
...
def test_unsubscribe_from_broadcast(client, mock_broadcast_service):
...
def test_unsubscribe_from_all(client, mock_broadcast_service):
...
def test_list_metrics(client, metric_data, mock_broadcast_service):
    """Test liste des métriques."""
    mock_broadcast_service.get_metrics.return_value = [metric_data]

    response = client.get("/v2/broadcast/metrics")

    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()) == 1


def test_list_metrics_with_period(client, mock_broadcast_service):
...
def test_record_metric(client, metric_data, mock_broadcast_service):
    """Test enregistrement de métriques."""
    mock_broadcast_service.record_metrics.return_value = metric_data

    response = client.post("/v2/broadcast/metrics/record")

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["period_type"] == "DAILY"


def test_record_metric_monthly(client, metric_data, mock_broadcast_service):
...
def test_get_dashboard_stats(client, dashboard_stats, mock_broadcast_service):
    """Test récupération des stats du dashboard."""
    mock_broadcast_service.get_dashboard_stats.return_value = dashboard_stats

    response = client.get("/v2/broadcast/dashboard")
...
def test_get_dashboard_stats_empty(client, mock_broadcast_service):
...
def test_get_due_broadcasts(client, broadcast_data, mock_broadcast_service):
    """Test récupération des diffusions à traiter."""
    mock_broadcast_service.get_broadcasts_due.return_value = [broadcast_data]

    response = client.get("/v2/broadcast/due")

    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()) == 1


def test_get_due_broadcasts_empty(client, mock_broadcast_service):
...
def test_process_due_broadcasts(client, broadcast_data, execution_data, mock_broadcast_service):
    """Test traitement des diffusions en attente."""
    mock_broadcast_service.get_broadcasts_due.return_value = [broadcast_data]
    mock_broadcast_service.execute_broadcast.return_value = execution_data

    response = client.post("/v2/broadcast/process-due")

    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()) == 1


def test_process_due_broadcasts_with_error(client, broadcast_data, mock_broadcast_service):
