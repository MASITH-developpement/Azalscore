"""
AZALS MODULE 14 - Subscriptions Router v2 (CORE SaaS)
======================================================
...
router = APIRouter(prefix="/v2/subscriptions", tags=["Subscriptions v2 - CORE SaaS"])
...
def list_all_addons(
    ...
    # Cette fonction n'existe pas dans le service original, on va retourner une liste vide
    # ou vous pouvez implémenter une méthode list_all_addons dans le service
    return []
...
def delete_addon(
    ...
    # Il faudrait l'implémenter dans le service
...
@router.post("/{subscription_id}/usage", response_model=UsageRecordResponse, status_code=201)
...
@cached(ttl=300, key_builder=lambda service: f"subscriptions_v2:stats:{service.tenant_id}")
def _get_cached_stats_v2(service: SubscriptionService) -> dict:
    """Calcule les stats abonnements v2 (cache 5min)."""
...
@cached(ttl=300, key_builder=lambda service: f"subscriptions_v2:dashboard:{service.tenant_id}")
def _get_cached_dashboard_v2(service: SubscriptionService) -> dict:
    """Calcule le dashboard abonnements v2 (cache 5min)."""
    dashboard = service.get_dashboard()
...
def get_stats(
    ...
    data = _get_cached_stats_v2(service)
...
def get_dashboard(
    ...
    data = _get_cached_dashboard_v2(service)
