...
class TicketCategory(Base):
    ...
    # Configuration
    default_priority: Mapped[str | None] = mapped_column(Enum(TicketPriority), default=TicketPriority.MEDIUM)
    ...
    # Options
    is_public: Mapped[bool | None] = mapped_column(Boolean, default=True)  # Visible clients
    require_approval: Mapped[bool | None] = mapped_column(Boolean, default=False)
    auto_assign: Mapped[bool | None] = mapped_column(Boolean, default=True)
...
class HelpdeskTeam(Base):
    ...
    auto_assign_method: Mapped[str | None] = mapped_column(String(20), default="round_robin")  # round_robin, least_busy, random

    # Paramètres
    max_tickets_per_agent: Mapped[int | None] = mapped_column(Integer, default=20)
    working_hours: Mapped[dict | None] = mapped_column(JSON)  # {mon: {start: "09:00", end: "18:00"}, ...}
...
class HelpdeskAgent(Base):
    ...
    # Profil
    display_name: Mapped[str | None] = mapped_column(String(255), nullable=False)
    email: Mapped[str | None] = mapped_column(String(255))
    avatar_url: Mapped[str | None] = mapped_column(String(500))
    signature: Mapped[str | None] = mapped_column(Text)

    # Statut
    status: Mapped[str | None] = mapped_column(Enum(AgentStatus), default=AgentStatus.OFFLINE)
    last_seen: Mapped[datetime | None] = mapped_column(DateTime)

    # Compétences
    skills: Mapped[dict | None] = mapped_column(JSON)  # ["billing", "technical", "sales"]
    languages: Mapped[dict | None] = mapped_column(JSON, default=["fr"])  # ["fr", "en", "es"]

    # Permissions
    can_assign: Mapped[bool | None] = mapped_column(Boolean, default=True)
    can_merge: Mapped[bool | None] = mapped_column(Boolean, default=False)
    can_delete: Mapped[bool | None] = mapped_column(Boolean, default=False)
    can_view_all: Mapped[bool | None] = mapped_column(Boolean, default=False)
    is_supervisor: Mapped[bool | None] = mapped_column(Boolean, default=False)

    # Stats
    tickets_assigned: Mapped[int | None] = mapped_column(Integer, default=0)
    tickets_resolved: Mapped[int | None] = mapped_column(Integer, default=0)
    avg_resolution_time: Mapped[int | None] = mapped_column(Integer, default=0)  # En minutes
    satisfaction_score: Mapped[Decimal | None] = mapped_column(Numeric(3, 2), default=0)
...
class HelpdeskSLA(Base):
    ...
    first_response_low: Mapped[int | None] = mapped_column(Integer, default=1440)  # 24h
    first_response_medium: Mapped[int | None] = mapped_column(Integer, default=480)  # 8h
    first_response_high: Mapped[int | None] = mapped_column(Integer, default=120)  # 2h
    first_response_urgent: Mapped[int | None] = mapped_column(Integer, default=60)  # 1h
    first_response_critical: Mapped[int | None] = mapped_column(Integer, default=15)  # 15min

    # Temps de résolution (en minutes)
    resolution_low: Mapped[int | None] = mapped_column(Integer, default=10080)  # 7 jours
    resolution_medium: Mapped[int | None] = mapped_column(Integer, default=2880)  # 2 jours
    resolution_high: Mapped[int | None] = mapped_column(Integer, default=1440)  # 1 jour
    resolution_urgent: Mapped[int | None] = mapped_column(Integer, default=480)  # 8h
    resolution_critical: Mapped[int | None] = mapped_column(Integer, default=120)  # 2h

    # Heures ouvrées
    business_hours_only: Mapped[bool | None] = mapped_column(Boolean, default=True)
    working_hours: Mapped[dict | None] = mapped_column(JSON)  # {mon: {start: "09:00", end: "18:00"}, ...}
    timezone: Mapped[str | None] = mapped_column(String(50), default="Europe/Paris")
    holidays: Mapped[dict | None] = mapped_column(JSON)  # ["2024-12-25", ...]

    # Escalade
    escalation_enabled: Mapped[bool | None] = mapped_column(Boolean, default=True)
    escalation_rules: Mapped[dict | None] = mapped_column(JSON)  # [{delay_minutes, action, target}, ...]
...
class Ticket(Base):
    ...
    # Identification
    ticket_number: Mapped[str | None] = mapped_column(String(50), nullable=False, unique=True)
    ...
    # Contenu
    subject: Mapped[str | None] = mapped_column(String(500), nullable=False)
    description: Mapped[str | None] = mapped_column(Text)
    source: Mapped[str | None] = mapped_column(Enum(TicketSource), default=TicketSource.WEB)

    # Statut et priorité
    status: Mapped[str | None] = mapped_column(Enum(TicketStatus), nullable=False, default=TicketStatus.NEW)
    priority: Mapped[str | None] = mapped_column(Enum(TicketPriority), nullable=False, default=TicketPriority.MEDIUM)
    ...
    # SLA tracking
    first_response_due: Mapped[datetime | None] = mapped_column(DateTime)
    ...
    sla_breached: Mapped[bool | None] = mapped_column(Boolean, default=False)

    # Tags et metadata
    tags: Mapped[dict | None] = mapped_column(JSON)  # ["urgent", "vip", "billing"]
    custom_fields: Mapped[dict | None] = mapped_column(JSON)  # Champs personnalisés
    ...
    internal_note_count: Mapped[int | None] = mapped_column(Integer, default=0)
...
class TicketReply(Base):
    ...
    # Auteur
    author_type: Mapped[str | None] = mapped_column(String(20), nullable=False)  # agent, customer, system
    ...
    # Type
    is_internal: Mapped[bool | None] = mapped_column(Boolean, default=False)  # Note interne
    is_first_response: Mapped[bool | None] = mapped_column(Boolean, default=False)
...
class TicketAttachment(Base):
    ...
    file_size: Mapped[int | None] = mapped_column(Integer)  # En bytes
...
class TicketHistory(Base):
    ...
    # Changement
    action: Mapped[str | None] = mapped_column(String(50), nullable=False)  # created, status_changed, assigned, etc.
    ...
    # Auteur
    actor_type: Mapped[str | None] = mapped_column(String(20))  # agent, customer, system, automation
...
class CannedResponse(Base):
    ...
    shortcut: Mapped[str | None] = mapped_column(String(50))  # Ex: #greeting
    ...
    # Scope
    scope: Mapped[str | None] = mapped_column(String(20), default="team")  # personal, team, global
    team_id: Mapped[uuid.UUID] = mapped_column(UniversalUUID(), ForeignKey("helpdesk_teams.id"))
...
class KBArticle(Base):
    ...
    slug: Mapped[str | None] = mapped_column(String(500), nullable=False)
    ...
    # Statut
    status: Mapped[str | None] = mapped_column(String(20), default="draft")  # draft, published, archived
    ...
    not_helpful_count: Mapped[int | None] = mapped_column(Integer, default=0)
...
class HelpdeskAutomation(Base):
    ...
    # Déclencheur
    trigger_type: Mapped[str | None] = mapped_column(String(50), nullable=False)  # ticket_created, ticket_updated, time_based
    trigger_conditions: Mapped[dict | None] = mapped_column(JSON)  # [{field, operator, value}, ...]

    # Actions
    actions: Mapped[dict | None] = mapped_column(JSON)  # [{type, params}, ...]

    # Priorité d'exécution
    priority: Mapped[int | None] = mapped_column(Integer, default=0)

    # Stats
    execution_count: Mapped[int | None] = mapped_column(Integer, default=0)
