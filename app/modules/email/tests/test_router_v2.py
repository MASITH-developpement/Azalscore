...
def test_create_config_success(client, mock_email_service, email_config_data, email_config):
    ...
    response = client.post("/v2/email/config", json=email_config_data)
...
def test_create_config_already_exists(client, mock_email_service, email_config_data, email_config):
...
def test_get_config_success(client, mock_email_service, email_config):
    """Test: Récupérer configuration email avec succès"""
    mock_email_service.get_config.return_value = MagicMock(**email_config)

    response = client.get("/v2/email/config")
...
def test_get_config_not_found(client, mock_email_service):
...
def test_update_config_success(client, mock_email_service, email_config):
    ...
    response = client.patch("/v2/email/config", json=update_data)
...
def test_update_config_not_found(client, mock_email_service):
    """Test: Mettre à jour configuration inexistante"""
    mock_email_service.update_config.return_value = None

    response = client.patch("/v2/email/config", json={"from_name": "New Name"})

    assert response.status_code == 404


def test_verify_config_success(client, mock_email_service):
    ...
    response = client.post("/v2/email/config/verify")
...
def test_verify_config_failure(client, mock_email_service):
...
def test_create_template_success(client, mock_email_service, email_template_data, email_template):
    """Test: Créer template email avec succès"""
    mock_email_service.create_template.return_value = MagicMock(**email_template)

    response = client.post("/v2/email/templates", json=email_template_data)
...
def test_list_templates_success(client, mock_email_service, email_template_list):
    """Test: Lister templates email"""
    mock_email_service.list_templates.return_value = [MagicMock(**t) for t in email_template_list]

    response = client.get("/v2/email/templates")
...
def test_list_templates_filtered_by_type(client, mock_email_service, email_template):
    """Test: Lister templates filtrés par type"""
    mock_email_service.list_templates.return_value = [MagicMock(**email_template)]

    response = client.get("/v2/email/templates?email_type=TRANSACTIONAL")
...
def test_get_template_success(client, mock_email_service, email_template):
    """Test: Récupérer template par ID"""
    template_id = email_template["id"]
    mock_email_service.get_template.return_value = MagicMock(**email_template)

    response = client.get(f"/v2/email/templates/{template_id}")
...
def test_get_template_not_found(client, mock_email_service):
    """Test: Récupérer template inexistant"""
    mock_email_service.get_template.return_value = None

    response = client.get("/v2/email/templates/nonexistent-id")

    assert response.status_code == 404


def test_update_template_success(client, mock_email_service, email_template):
    ...
    response = client.patch(f"/v2/email/templates/{template_id}", json=update_data)
...
def test_update_template_not_found(client, mock_email_service):
    """Test: Mettre à jour template inexistant"""
    mock_email_service.update_template.return_value = None

    response = client.patch("/v2/email/templates/nonexistent-id", json={"name": "Updated"})
...
def test_send_email_success(client, mock_email_service, send_email_data, send_email_response):
    """Test: Envoyer email avec succès"""
    mock_email_service.send_email.return_value = MagicMock(**send_email_response)

    response = client.post("/v2/email/send", json=send_email_data)
...
def test_send_bulk_success(client, mock_email_service, bulk_send_data, bulk_send_response):
    """Test: Envoi en masse avec succès"""
    mock_email_service.send_bulk.return_value = MagicMock(**bulk_send_response)

    response = client.post("/v2/email/send/bulk", json=bulk_send_data)
...
def test_process_queue_success(client, mock_email_service):
    """Test: Traiter la file d'attente"""
    mock_email_service.process_queue.return_value = 5

    response = client.post("/v2/email/queue/process?batch_size=10")
...
def test_process_queue_with_default_batch_size(client, mock_email_service):
    """Test: Traiter la file avec batch size par défaut"""
    mock_email_service.process_queue.return_value = 3

    response = client.post("/v2/email/queue/process")
...
def test_list_logs_filtered_by_status(client, mock_email_service, email_log):
    """Test: Lister logs filtrés par status"""
    mock_email_service.list_logs.return_value = ([MagicMock(**email_log)], 1)

    response = client.get("/v2/email/logs?status=SENT")
...
def test_list_logs_filtered_by_type(client, mock_email_service, email_log):
    """Test: Lister logs filtrés par type"""
    mock_email_service.list_logs.return_value = ([MagicMock(**email_log)], 1)

    response = client.get("/v2/email/logs?email_type=TRANSACTIONAL")
...
def test_list_logs_with_pagination(client, mock_email_service, email_log_list):
    """Test: Lister logs avec pagination"""
    mock_email_service.list_logs.return_value = ([MagicMock(**email_log_list[0])], 2)

    response = client.get("/v2/email/logs?skip=0&limit=1")
...
def test_get_log_success(client, mock_email_service, email_log):
    """Test: Récupérer log email par ID"""
    log_id = email_log["id"]
    mock_email_service.get_log.return_value = MagicMock(**email_log)

    response = client.get(f"/v2/email/logs/{log_id}")
...
def test_get_log_not_found(client, mock_email_service):
    """Test: Récupérer log inexistant"""
    mock_email_service.get_log.return_value = None

    response = client.get("/v2/email/logs/nonexistent-id")
...
def test_get_dashboard_success(client, mock_email_service, email_dashboard):
    """Test: Récupérer dashboard email"""
    mock_email_service.get_dashboard.return_value = MagicMock(**email_dashboard)

    response = client.get("/v2/email/dashboard")
...
def test_context_isolation(client, mock_email_service, tenant_id, user_id):
...
def test_audit_trail_with_user_id(client, mock_email_service, send_email_data, send_email_response, user_id):
