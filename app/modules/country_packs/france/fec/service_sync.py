"""
AZALS MODULE - FEC: Service Synchrone
=====================================

Service de génération et validation du FEC conforme à l'Article A.47 A-1 du LPF.
Version synchrone compatible avec SQLAlchemy Session.
...
class FECServiceSync:
    """
    Service de génération et validation FEC (version synchrone).
    ...
    def generate_fec(self, request: FECExportRequest) -> FECExport:
        """
        ...
        fiscal_year = self._get_fiscal_year(request.fiscal_year_id)
        if not fiscal_year:
        ...
        try:
            ...
            fec_content, statistics = self._generate_fec_content(
                entries=entries,
            ...
            validation_result = self._validate_fec_content(fec_content, export)
    ...
    def _get_fiscal_year(self, fiscal_year_id: UUID) -> Optional[FiscalYear]:
        ...
        for entry in entries:
            ...
            for line in entry.lines:
                ...
                fec_line = FECLine(
                    ...
                    CompAuxNum=str(line.partner_id)[:50] if line.partner_id else None,
    ...
    def validate_fec(self, export_id: UUID) -> FECValidationResponse:
        """
        ...
        # NOTE: Phase 2 - Intégration storage_service pour récupération
        raise NotImplementedError("Validation d'export existant non implémentée")

    def validate_fec_content(self, content: str) -> FECValidationResponse:
        """
        ...
        Returns:
            FECValidationResponse: Résultat de validation
        """
        return self._validate_fec_content(content, None)

    def _validate_fec_content(
        self,
        content: str,
    ...
    def _validate_line(
        ...
        # Valider les dates
        for date_value, col_name in [
            (entry_date, "EcritureDate"),
    ...
    def _get_export(self, export_id: UUID) -> Optional[FECExport]:
        """Récupère un export FEC."""
        return self.db.query(FECExport).filter(
            and_(
    ...
    def list_exports(
        ...
        query = self.db.query(FECExport).filter(FECExport.tenant_id == self.tenant_id)

        if fiscal_year_code:
            query = query.filter(FECExport.fiscal_year_code == fiscal_year_code)
        if status:
            query = query.filter(FECExport.status == status)

        # Count
        total = query.count()

        # Results
        items = query.order_by(FECExport.requested_at.desc()).offset(skip).limit(limit).all()

        return items, total

    def get_export(self, export_id: UUID) -> Optional[FECExport]:
        """Récupère un export FEC par ID."""
        return self._get_export(export_id)

    def download_export(self, export_id: UUID) -> tuple[str, bytes]:
        """
        ...
        # NOTE: Phase 2 - Intégration storage_service
    ...
    def archive_export(self, export_id: UUID) -> FECArchive:
        """
