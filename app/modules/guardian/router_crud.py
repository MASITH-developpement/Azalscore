"""
AZALS MODULE GUARDIAN - Router Unifié
=====================================
...
async def update_config(
    ...
    """Met à jour la configuration GUARDIAN. Réservé aux rôles DIRIGEANT et ADMIN."""
...
async def report_frontend_error(
    ...
    """Signale une erreur frontend. Les données utilisateur sont automatiquement pseudonymisées."""
    service = get_guardian_service(db, context.tenant_id)
    try:
...
async def validate_correction(
    ...
    """Valide ou rejette une correction en attente. Réservé aux rôles DIRIGEANT et ADMIN."""
    if context.role not in [UserRole.DIRIGEANT, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def rollback_correction(
    ...
    """Effectue un rollback d'une correction. Réservé aux rôles DIRIGEANT et ADMIN."""
    if context.role not in [UserRole.DIRIGEANT, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def create_rule(
    ...
    """Crée une nouvelle règle de correction automatique. Réservé aux rôles DIRIGEANT et ADMIN."""
    if context.role not in [UserRole.DIRIGEANT, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def list_rules(
    ...
    """Liste les règles de correction avec filtres."""
    service = get_guardian_service(db, context.tenant_id)
...
async def update_rule(
    ...
    """Met à jour une règle de correction. Réservé aux rôles DIRIGEANT et ADMIN."""
    if context.role not in [UserRole.DIRIGEANT, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def delete_rule(
    ...
    """Supprime (désactive) une règle de correction. Réservé aux rôles DIRIGEANT et ADMIN."""
    if context.role not in [UserRole.DIRIGEANT, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def list_alerts(
    ...
    """Liste les alertes GUARDIAN avec filtres."""
    service = get_guardian_service(db, context.tenant_id)
...
async def get_dashboard(
    ...
    """Récupère le dashboard GUARDIAN complet."""
    service = get_guardian_service(db, context.tenant_id)
...
async def export_audit(
    ...
    """Exporte l'historique d'audit GUARDIAN."""
...
async def get_incident_screenshot(
    ...
    """Récupère le screenshot d'un incident."""
...
async def list_daily_reports(
    ...
    """Liste les rapports quotidiens GUARDIAN."""
    service = get_guardian_service(db, context.tenant_id)
...
async def get_daily_report(
    ...
    """Récupère un rapport quotidien spécifique."""
    service = get_guardian_service(db, context.tenant_id)
...
async def generate_daily_report(
    ...
    """Génère un rapport quotidien GUARDIAN. Réservé aux rôles DIRIGEANT et ADMIN."""
    if context.role not in [UserRole.DIRIGEANT, UserRole.ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
