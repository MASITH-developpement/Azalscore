...
class POSStore(Base):
    ...
    default_tax_rate: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), default=20.00)

    # Horaires
    opening_hours: Mapped[Optional[dict]] = mapped_column(JSON)  # {"mon": {"open": "09:00", "close": "19:00"}, ...}

    # Paramètres
    settings: Mapped[Optional[dict]] = mapped_column(JSON)  # Options diverses
...
class POSTerminal(Base):
    ...
    # Identification
    terminal_id: Mapped[Optional[str]] = mapped_column(String(50), nullable=False)  # Identifiant unique du terminal
    name: Mapped[Optional[str]] = mapped_column(String(100), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text)

    # Hardware
    device_type: Mapped[Optional[str]] = mapped_column(String(50))  # ipad, android, desktop, dedicated
    device_id: Mapped[Optional[str]] = mapped_column(String(255))  # Identifiant hardware unique
    printer_ip: Mapped[Optional[str]] = mapped_column(String(50))
    drawer_ip: Mapped[Optional[str]] = mapped_column(String(50))

    # Statut
    status: Mapped[Optional[str]] = mapped_column(Enum(POSTerminalStatus), default=POSTerminalStatus.INACTIVE)
...
class POSUser(Base):
    ...
    # Informations
    employee_code: Mapped[Optional[str]] = mapped_column(String(20), nullable=False)
    ...
    pin_code: Mapped[Optional[str]] = mapped_column(String(10))  # Code PIN pour connexion rapide

    # Droits
    can_open_drawer: Mapped[Optional[bool]] = mapped_column(Boolean, default=True)
    can_void_transaction: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)
    can_give_discount: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)
    max_discount_percent: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), default=0)
    can_refund: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)
    can_close_session: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)
    is_manager: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)

    # Magasins autorisés
    allowed_store_ids: Mapped[Optional[dict]] = mapped_column(JSON)  # [1, 2, 3]
...
class POSSession(Base):
    ...
    # Numéro de session
    session_number: Mapped[Optional[str]] = mapped_column(String(50), nullable=False)
    ...
    # Statut
    status: Mapped[Optional[str]] = mapped_column(Enum(POSSessionStatus), default=POSSessionStatus.OPEN)

    # Fond de caisse
    opening_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=False, default=0)
    opening_note: Mapped[Optional[str]] = mapped_column(Text)

    # Clôture
    expected_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))  # Calculé
    actual_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))  # Compté
    cash_difference: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))  # Écart
    closing_note: Mapped[Optional[str]] = mapped_column(Text)

    # Totaux calculés
    total_sales: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    total_refunds: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    total_discounts: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    total_tax: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    transaction_count: Mapped[Optional[int]] = mapped_column(Integer, default=0)

    # Par mode de paiement
    cash_total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    card_total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    check_total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    voucher_total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    other_total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)

    # Dates
    opened_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=datetime.utcnow)
...
class CashMovement(Base):
    ...
    # Type
    movement_type: Mapped[Optional[str]] = mapped_column(String(20), nullable=False)  # IN, OUT

    # Montant
    amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=False)

    # Motif
    reason: Mapped[Optional[str]] = mapped_column(String(100), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text)
...
class POSTransaction(Base):
    ...
    # Numéro de ticket
    receipt_number: Mapped[Optional[str]] = mapped_column(String(50), nullable=False)
    receipt_sequence: Mapped[int] = mapped_column(Integer)

    # Statut
    status: Mapped[Optional[str]] = mapped_column(Enum(POSTransactionStatus), default=POSTransactionStatus.PENDING)
    ...
    # Montants
    subtotal: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=False, default=0)
    ...
    total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=False, default=0)

    # Paiement
    amount_paid: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    amount_due: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    change_given: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)

    # Remise globale
    discount_type: Mapped[Optional[str]] = mapped_column(Enum(DiscountType))
    discount_value: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))
    ...
    offline_id: Mapped[Optional[str]] = mapped_column(String(100))  # ID généré offline
    ...
    void_reason: Mapped[Optional[str]] = mapped_column(String(255))
...
class POSTransactionLine(Base):
    ...
    # Numéro de ligne
    line_number: Mapped[Optional[int]] = mapped_column(Integer, default=1)
    ...
    # Quantité
    quantity: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 3), nullable=False)
    ...
    original_price: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))  # Prix avant remise
    ...
    # Taxes
    tax_rate: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), default=20.00)
    tax_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)

    # Total
    line_total: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), nullable=False)
    ...
    # Retour
    is_return: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)
...
class POSPayment(Base):
    ...
    # Type de paiement
    payment_method: Mapped[Optional[str]] = mapped_column(Enum(PaymentMethodType), nullable=False)
    payment_method_name: Mapped[Optional[str]] = mapped_column(String(100))  # Détail (ex: "Visa", "Mastercard")
    ...
    card_auth_code: Mapped[Optional[str]] = mapped_column(String(50))
    card_transaction_id: Mapped[Optional[str]] = mapped_column(String(100))

    # Détails chèque
    check_number: Mapped[Optional[str]] = mapped_column(String(50))
    check_bank: Mapped[Optional[str]] = mapped_column(String(100))

    # Détails carte cadeau
    gift_card_number: Mapped[Optional[str]] = mapped_column(String(100))

    # Rendu monnaie (pour espèces)
    amount_tendered: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))
    change_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))

    # Statut
    status: Mapped[Optional[str]] = mapped_column(String(20), default="COMPLETED")  # COMPLETED, PENDING, FAILED, REFUNDED
...
class POSDailyReport(Base):
    ...
    report_number: Mapped[Optional[str]] = mapped_column(String(50), nullable=False)

    # Périodes
    first_transaction_at: Mapped[Optional[datetime]] = mapped_column(DateTime)
    last_transaction_at: Mapped[Optional[datetime]] = mapped_column(DateTime)

    # Totaux
    gross_sales: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    net_sales: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    total_discounts: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    total_refunds: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    total_tax: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    ...
    # Transactions
    transaction_count: Mapped[Optional[int]] = mapped_column(Integer, default=0)
    items_sold: Mapped[Optional[int]] = mapped_column(Integer, default=0)
    average_transaction: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    ...
    # Caisse
    opening_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    closing_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    cash_movements_in: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    cash_movements_out: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)

    # Écarts
    expected_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    actual_cash: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)
    cash_variance: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2), default=0)

    # Détails par TVA
    tax_breakdown: Mapped[Optional[dict]] = mapped_column(JSON)  # {"20": {"base": 1000, "tax": 200}, "5.5": {...}}

    # Détails par catégorie
    category_breakdown: Mapped[Optional[dict]] = mapped_column(JSON)
    ...
    generated_at: Mapped[Optional[datetime]] = mapped_column(DateTime, default=datetime.utcnow)
...
class POSProductQuickKey(Base):
    ...
    # Position
    page: Mapped[Optional[int]] = mapped_column(Integer, default=1)
    ...
    color: Mapped[Optional[str]] = mapped_column(String(20), default="#1976D2")
    icon: Mapped[Optional[str]] = mapped_column(String(50))
    image_url: Mapped[Optional[str]] = mapped_column(String(500))

    # Prix personnalisé (optionnel)
    custom_price: Mapped[Optional[Decimal]] = mapped_column(Numeric(15, 2))
...
class POSHoldTransaction(Base):
    ...
    # Identification
    hold_number: Mapped[Optional[str]] = mapped_column(String(50), nullable=False)
    hold_name: Mapped[Optional[str]] = mapped_column(String(100))  # Nom du client ou référence
...
class POSOfflineQueue(Base):
    ...
    # Type d'opération
    operation_type: Mapped[Optional[str]] = mapped_column(String(50))  # TRANSACTION, PAYMENT, CASH_MOVEMENT

    # Données
    transaction_data: Mapped[dict] = mapped_column(JSON)
    offline_id: Mapped[Optional[str]] = mapped_column(String(100))

    # Statut sync
    is_synced: Mapped[Optional[bool]] = mapped_column(Boolean, default=False)
    sync_attempts: Mapped[Optional[int]] = mapped_column(Integer, default=0)
