"""
AZALS - Endpoints Authentification SÉCURISÉS (MIGRÉ CORE SaaS)
===============================================================
...
- Endpoints PROTÉGÉS migrés vers get_saas_context()
- Endpoints PUBLICS (login, register, bootstrap) gardent pattern actuel
- 9/15 endpoints migrés (endpoints protégés nécessitant JWT)
- 6/15 endpoints publics gardés (pas de JWT requis)

ENDPOINTS PUBLICS (pas de migration):
- /auth/register, /auth/login, /auth/bootstrap, /auth/refresh
- /auth/2fa/verify-login, /auth/force-change-password

ENDPOINTS PROTÉGÉS (migrés vers CORE):
- /auth/2fa/setup, /auth/2fa/enable, /auth/2fa/disable, /auth/2fa/status
- /auth/2fa/regenerate-backup-codes, /auth/logout, /auth/me
- /auth/capabilities, /auth/change-password
"""
...
# ============================================================================
# ENDPOINTS PUBLICS (NON MIGRÉS - PAS DE JWT REQUIS)
# ============================================================================
...
def register(
    ...
    Utilise get_tenant_id() seulement.
    """
...
# ============================================================================
# ENDPOINTS 2FA (Two-Factor Authentication) - MIGRÉS CORE SaaS
# ============================================================================
...
def setup_2fa(
    ...
    - Charge User depuis DB pour accès totp_enabled
    """
    ...
    result = two_factor_service.setup_2fa(user)
...
def enable_2fa(
    ...
    two_factor_service = TwoFactorService(db)
    result = two_factor_service.verify_and_enable_2fa(user, verify_request.code)
...
def verify_2fa_login(
    ...
    NOTE: Endpoint PUBLIC - pas de migration CORE (utilise pending token, pas JWT).
    """
...
def disable_2fa(
    ...
    two_factor_service = TwoFactorService(db)
    result = two_factor_service.disable_2fa(user, verify_request.code)
...
def get_2fa_status(
    ...
    two_factor_service = TwoFactorService(db)
    status_data = two_factor_service.get_2fa_status(user)
...
def regenerate_backup_codes(
    ...
    two_factor_service = TwoFactorService(db)
    success, new_codes = two_factor_service.regenerate_backup_codes(user, verify_request.code)
...
def logout(
    ...
    ✅ MIGRÉ CORE SaaS:
    - Utilise context (pas besoin de charger User)
    - Révocation token identique
    """
...
# ===== REFRESH TOKEN - PUBLIC (NON MIGRÉ) =====
...
def refresh_access_token(
    ...
    NOTE: Endpoint PUBLIC - pas de migration CORE (utilise refresh token, pas JWT actif).
    """
    from app.core.security import decode_access_token
...
# ===== ENDPOINTS UTILISATEUR COURANT - MIGRÉS CORE SaaS =====
...
def get_current_user_info(
    ...
    ✅ MIGRÉ CORE SaaS:
    - Utilise context.user_id
    - Charge User depuis DB pour infos complètes
    """
    from app.core.logging_config import get_logger
    ...
    try:
        ...
        if not user:
            logger.warning(
                "[GUARDIAN] /auth/me called but user not found in DB",
                extra={"client_ip": client_ip, "user_id": str(context.user_id), "path": "/auth/me"}
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="User not found"
...
def get_user_capabilities(
    ...
    ✅ MIGRÉ CORE SaaS:
    - Utilise context.role directement
    - NOTE: À terme, utiliser context.permissions au lieu de role-based capabilities
    """
    ...
    role_name = context.role.value if hasattr(context.role, 'value') else str(context.role)
...
# ===== CHANGEMENT DE MOT DE PASSE - MIGRÉ CORE SaaS =====
...
def force_change_password(
    ...
    NOTE: Endpoint semi-public - pas de migration CORE (utilise token manuel).
    """
    from datetime import datetime
